...
export const step1Schema = step1Base.superRefine((data, ctx) => {
...
export const step3Schema = step3Base.superRefine((data, ctx) => {
...
// ========== SCHEMA COMPLETO==========
// Merging base objects first, then applying refinements
export const criarEventoSchema = step1Base
  .merge(step3Base)
  .superRefine((data, ctx) => {
    // Re-applying refinements from Step 1
    if (data.dataInicio && data.dataFim) {
      const start = new Date(data.dataInicio);
      const end = new Date(data.dataFim);
      if (!isNaN(start.getTime()) && !isNaN(end.getTime()) && start > end) {
    ...
    // Re-applying refinements from Step 3
    if (data.exibInicio && data.exibFim) {
      const start = new Date(data.exibInicio);
      const end = new Date(data.exibFim);
      if (!isNaN(start.getTime()) && !isNaN(end.getTime()) && start > end) {
